...
if [ ! -f /sentinel/neobytes.conf ]; then
  if [ -z "$RPCUSER" -o -z "$RPCPASSWORD" -o -z "$RPCPORT" ]; then
    echo "When no /sentinel/neobytes.conf is present, you must at least set RPCUSER, RPCPORT and RPCPASSWORD environment variables"
    exit 1
  fi

  echo "" > /sentinel/neobytes.conf
  if [ -n "$RPCUSER" ]; then
    echo "rpcuser=${RPCUSER}" >> /sentinel/neobytes.conf
  fi
  if [ -n "$RPCPASSWORD" ]; then
    echo "rpcpassword=${RPCPASSWORD}" >> /sentinel/neobytes.conf
  fi
  if [ -n "$RPCPORT" ]; then
    echo "rpcport=${RPCPORT}" >> /sentinel/neobytes.conf
  fi
fi
...
if [ ! -f /sentinel/sentinel.conf ]; then
  ...
  echo "neobytes_conf=/sentinel/neobytes.conf" > /sentinel/sentinel.conf
  if [ -n "$RPCHOST" ]; then
