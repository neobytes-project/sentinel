...
def is_valid_neobytes_address(address, network='mainnet'):
    ...
    neobytes_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Neobytes address
        return False

    if (address_version != neobytes_version):
        return False
...
# shims 'til we can fix the neobytesd side
def SHIM_serialise_for_neobytesd(sentinel_hex):
    from models import NEOBYTESD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = NEOBYTESD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in neobytesd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # neobytesd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    neobytesd_hex = serialise(obj)
    return neobytesd_hex


# shims 'til we can fix the neobytesd side
def SHIM_deserialise_from_neobytesd(neobytesd_hex):
    from models import NEOBYTESD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(neobytesd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('neobytes.conf')
