...
class NeoBytesConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read neobytes.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is neobytes.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from neobytes.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard NeoBytes defaults...
        default_port = 11426 if (network == 'mainnet') else 12426

        # use default port for network if not specified in neobytes.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from neobytes.conf
