...
def has_neobytes_conf():
    import config
    import io

    valid_neobytes_conf = False

    # ensure neobytes_conf exists & readable
    #
    # if not, print a message stating that NeoBytes Core must be installed and
    # configured, including JSONRPC access in neobytes.conf
    try:
        f = io.open(config.neobytes_conf)
        valid_neobytes_conf = True
    except IOError as e:
        print(e)

    return valid_neobytes_conf
...
def main():
    ...
    if not has_neobytes_conf():
        print("NeoBytesCore must be installed and configured, including JSONRPC access in neobytes.conf")
        sys.exit(1)
