...
sentinel_cfg = NeoBytesConfig.tokenize(sentinel_config_file)
sentinel_version = "1.5.0"


def get_neobytes_conf():
    if sys.platform == 'win32':
        neobytes_conf = os.path.join(os.getenv('APPDATA'), "NeoBytesCore/neobytes.conf")
    else:
        home = os.environ.get('HOME')

        neobytes_conf = os.path.join(home, ".neobytescore/neobytes.conf")
        if sys.platform == 'darwin':
            neobytes_conf = os.path.join(home, "Library/Application Support/NeoBytesCore/neobytes.conf")

    neobytes_conf = sentinel_cfg.get('neobytes_conf', neobytes_conf)

    return neobytes_conf
...
neobytes_conf = get_neobytes_conf()
network = get_network()
