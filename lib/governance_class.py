...
class GovernanceClass(object):
    ...
    def vote(self, neobytesd, signal, outcome):
        return self.go.vote(neobytesd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, neobytesd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(neobytesd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(neobytesd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to neobytesd "gobject list" output
    ...
    def neobytesd_serialise(self):
        import neobyteslib
        return neobyteslib.SHIM_serialise_for_neobytesd(self.serialise())
