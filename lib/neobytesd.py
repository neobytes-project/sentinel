...
class NeobytesDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some neobytesd methods
    ...
    @classmethod
    def from_neobytes_conf(self, neobytes_dot_conf):
        from neobytes_config import NeobytesConfig
        config_text = NeobytesConfig.slurp_config_file(neobytes_dot_conf)
        creds = NeobytesConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = neobyteslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = neobyteslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = neobyteslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_neobytesd_proto_version_with_sentinel_ping)
